...
def main(args):
    ...
    config = checkpoint['config'] if 'config' in checkpoint else config
    if args.steps:
        config['model']['num_denoising_steps'] = args.steps
        print("Overriding steps", args.steps)
    ...
    if args.vq_ckpt != "no":
        checkpoint = torch.load(args.vq_ckpt, map_location="cpu")
        print("use_ema:", args.use_ema)
        if args.use_ema and "ema" in checkpoint:  # ema
        ...
        print(vq_model.load_state_dict(model_weight, strict=False))
        del checkpoint
    ...
    if rank == 0:
        ...
        os.makedirs(f"{args.sample_dir}/gt", exist_ok=True)
    ...
    for x, _ in loader:
        ...
        with torch.no_grad(), torch.autocast("cuda", dtype=torch.bfloat16):
            samples = vq_model(x)
        if args.image_size_eval != args.image_size and samples.shape[2] != args.image_size_eval:
            samples = F.interpolate(samples, size=(args.image_size_eval, args.image_size_eval), mode='bicubic')
        samples = torch.clamp(127.5 * samples + 128.0, 0, 255).permute(0, 2, 3, 1).to("cpu", dtype=torch.uint8).numpy()
        # rgb_gts_to_save = torch.clamp(127.5 * rgb_gts + 128.0, 0, 255).permute(0, 2, 3, 1).to("cpu", dtype=torch.uint8).numpy()

        # Save samples to disk as individual .png files
        for i, (sample, rgb_gt) in enumerate(zip(samples, rgb_gts)):
            index = i * dist.get_world_size() + rank + total
            Image.fromarray(sample).save(f"{sample_folder_dir}/{index:06d}.png")
            Image.fromarray((rgb_gt*255.).astype(np.uint8)).save(f"{args.sample_dir}/gt/{index:06d}.png")
            # metric
