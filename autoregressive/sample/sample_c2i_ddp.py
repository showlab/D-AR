...
def main(args):
    ...
    if rank == 0:
        for i in clip_steps:
            os.makedirs(f"{sample_folder_dir}-{i}", exist_ok=True)
    ...
    for _ in pbar:
        ...
        for clip_step in clip_steps:
            # Save samples to disk as individual .png files
            samples = vq_model.decode_code(index_sample, clip_steps=clip_step) 
            ...
            for i, sample in enumerate(samples):
                ...
                Image.fromarray(sample).save(f"{sample_folder_dir}-{clip_step}/{index:06d}.png")
    ...
    if rank == 0:
        for clip_step in clip_steps:
            create_npz_from_sample_folder(f"{sample_folder_dir}-{clip_step}", args.num_fid_samples)
        print("Done.")
    dist.barrier()
